...
<body>
    ...
    <div class="board">
        <form class="d-flex justify-content-end m-3">
            <input class="form-control me-2 " type="search" placeholder="Search" aria-label="Search" style="width:30vw;" onkeyup="searchA(event)">
        ...
        <div class="lanes">
            <div class="swim-lane" id="todo-lane">
                ...
                <?php for ($i = 0; $i < count($to_do); $i++) { ?>
                    <form action="../controllers/task.php" method="post" class="task" draggable="true">
                        <input type="hidden" class="ID-TASK" name="ID" value="<?= $to_do[$i]['ID_TASK'] ?>">
                        <p class="title"><?= $to_do[$i]['TITLE'] ?></p>
                        <p><?php echo $to_do[$i]['DATE'] ?></p>
                        <input type="submit" value="delete" name="DELETETASK">
                    </form>
                <?php } ?>
            </div>
            ...
            <div class="swim-lane">
                ...
                <?php for ($i = 0; $i < count($doing); $i++) { ?>
                    <form action="../controllers/task.php" method="post" class="task" draggable="true">
                        <input type="hidden" class="ID-TASK" name="ID" value="<?= $doing[$i]['ID_TASK'] ?>">
                        <p class="title"><?= $doing[$i]['TITLE'] ?></p>
            ...
            <div class="swim-lane">
                ...
                <?php for ($i = 0; $i < count($done); $i++) { ?>
                    <form action="../controllers/task.php" method="post" class="task" draggable="true">
                        <input type="hidden" class="ID-TASK" name="ID" value="<?= $done[$i]['ID_TASK'] ?>">
                        <p class="title"><?= $done[$i]['TITLE'] ?></p>
